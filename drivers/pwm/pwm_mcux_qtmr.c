...
static int mcux_qtmr_pwm_set_cycles(const struct device *dev,
				       ...
				       uint32_t pulse_cycles, pwm_flags_t flags)
{
	...
	reg |= TMR_CTRL_CM(0x001);

	reg &= ~TMR_CTRL_PCS_MASK;
	reg |= TMR_CTRL_PCS(0x8 | (31 - __builtin_clz(config->prescaler)));
...
static int mcux_qtmr_pwm_get_cycles_per_sec(const struct device *dev,
					       uint32_t channel,
					       uint64_t *cycles)
{
	const struct pwm_mcux_qtmr_config *config = dev->config;
...
static int mcux_qtmr_pwm_init(const struct device *dev)
{
	...
	QTMR_Init(config->base, 0, &qtmr_config);
