...
static void f9p_rtk_data_cb(const struct device *dev, const struct gnss_rtk_data *data)
{
	 ...
	 */
	(void)u_blox_iface_msg_send(dev, (const void *)data->data, data->len, false);
}
...
static int u_blox_f9p_init(const struct device *dev)
{
	...
	err = u_blox_iface_init(dev, u_blox_f9p_unsol_messages,
...
#define UBX_F9P(inst)										   \
	...
	BUILD_ASSERT((DT_INST_PROP(inst, fix_rate) >= 50) &&					   \
		     ...
		     " and must fit in 16-bits.");						   \
												   \
	static const struct u_blox_iface_config u_blox_f9p_cfg_##inst = {			   \
		...
		.baudrate = {								   \
			...
			.desired = DT_PROP(DT_INST_BUS(inst), current_speed),		   \
												   ...
												   \
	static struct u_blox_iface_data u_blox_f9p_data_##inst;					   \
												   \
	...
	DEVICE_DT_INST_DEFINE(inst,								   \
			      u_blox_f9p_init,							   \
			      NULL,								   \
			      &u_blox_f9p_data_##inst,						   \
			      &u_blox_f9p_cfg_##inst,						   \
