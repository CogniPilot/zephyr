...
static int phy_tja1103_await_phyid(const struct device *dev)
{
	int ret;
	uint32_t count;
	uint32_t phy_id;
	 ...
	 */
	for (count = 0U; count < TJA1103_AWAIT_RETRY_COUNT; ++count) {
		ret = phy_tja1103_id(dev, &phy_id);
		if (ret >= 0) {
			if (phy_id == TJA1103_ID) {
				break;
			}
			ret = -ETIMEDOUT;
		}
...
static int phy_tja1103_init(const struct device *dev)
{
	...
	ret = phy_tja1103_await_phyid(dev);
	if (ret < 0) {
		LOG_ERR("PHY %u didn't come out of reset, %d", cfg->phy_addr, ret);
		return -ENODEV;
	}
