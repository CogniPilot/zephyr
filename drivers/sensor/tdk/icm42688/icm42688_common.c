...
int icm42688_configure(const struct device *dev, struct icm42688_cfg *cfg)
{
	...
	res = icm42688_spi_single_write(
		&dev_cfg->spi, REG_ACCEL_CONFIG_STATIC2, accel_config_static2);
	if (res != 0) {
	...
	uint8_t accel_config_static3 =
		FIELD_PREP(MASK_ACCEL_AAF_DELTSQR_7_0, (VAR_ACCEL_AAF_DELT*VAR_ACCEL_AAF_DELT));

	LOG_DBG("ACCEL_CONFIG_STATIC3 (0x%x) 0x%x", REG_ACCEL_CONFIG_STATIC3, accel_config_static3);
	res = icm42688_spi_single_write(
		&dev_cfg->spi, REG_ACCEL_CONFIG_STATIC3, accel_config_static3);
	if (res != 0) {
	...
	res = icm42688_spi_single_write(
		&dev_cfg->spi, REG_ACCEL_CONFIG_STATIC4, accel_config_static4);
	if (res != 0) {
	...
	res = icm42688_spi_single_write(
		&dev_cfg->spi, REG_GYRO_CONFIG_STATIC2, gyro_config_static2);
	if (res != 0) {
	...
	res = icm42688_spi_single_write(
		&dev_cfg->spi, REG_GYRO_CONFIG_STATIC3, gyro_config_static3);
	if (res != 0) {
	...
	uint8_t gyro_config_static4 =
		FIELD_PREP(MASK_GYRO_AAF_DELTSQR_7_0, (VAR_GYRO_AAF_DELT*VAR_GYRO_AAF_DELT));

	LOG_DBG("GYRO_CONFIG_STATIC4 (0x%x) 0x%x", REG_GYRO_CONFIG_STATIC4, gyro_config_static4);
	res = icm42688_spi_single_write(
		&dev_cfg->spi, REG_GYRO_CONFIG_STATIC4, gyro_config_static4);
	if (res != 0) {
	...
	res = icm42688_spi_single_write(
		&dev_cfg->spi, REG_GYRO_CONFIG_STATIC5, gyro_config_static5);
	if (res != 0) {
