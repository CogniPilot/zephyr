...
{
	struct icm45686_encoded_data *edata = (struct icm45686_encoded_data *)buffer;
	const uint8_t *frame_begin = (const uint8_t *)&edata->fifo_payload;
	struct icm45686_encoded_fifo_payload *fdata = &edata->fifo_payload;
	const uint8_t *buffer_end = (edata->header.fifo_count *
	...
	while (count < max_count && frame_begin < buffer_end) {
		const uint8_t *frame_end = (const uint8_t *)frame_begin + 20;

		fdata = (struct icm45686_encoded_fifo_payload *)frame_begin;
		...
		case SENSOR_CHAN_DIE_TEMP: {
			...
			out->readings[count].temperature =
				icm45686_fifo_read_temp_from_packet((uint8_t *)frame_begin);
			break;
		}
		default:
			return 0;
