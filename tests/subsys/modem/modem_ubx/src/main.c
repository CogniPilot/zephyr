...
static struct script_runner script_runner;
...
static inline void restore_ubx_script()
{
	...
	static const struct script_runner script_runner_restored = {
		.script = {
			.request = {
				.buf = &test_req,
				.len = UBX_FRM_SZ(frame_restored.payload_size),
			},
	...
	script_runner = script_runner_restored;
...
ZTEST_SUITE(modem_ubx, NULL, test_setup, test_before, test_after, NULL);
...
ZTEST(modem_ubx, test_cmd_no_rsp_is_non_blocking)
{
	...
	script_runner.script.match.filter.class = 0;
	...
	zassert_ok(script_runner.result.ret, "%d", script_runner.result.ret);
	zassert_equal(UBX_FRM_SZ(test_req.payload_size), len, "expected: %d, got: %d",
		      UBX_FRM_SZ(test_req.payload_size), len);
}
...
ZTEST(modem_ubx, test_cmd_rsp_retries_and_times_out)
{
	...
	script_runner.script.timeout = K_SECONDS(3);
	script_runner.script.retry_count = 2; /* 2 Retries -> 3 Tries */
	...
	for (size_t i = 0 ; i < (script_runner.script.retry_count + 1) ; i++) {

		int len = modem_backend_mock_get(&mock, buf, sizeof(buf));

		zassert_false(script_runner.result.done, "Script should not be done. "
							 "Iteration: %d", i);
		zassert_equal(UBX_FRM_SZ(test_req.payload_size), len,
			      "Payload Sent does not match. "
			      "Expected: %d, Received: %d, Iteration: %d",
			      UBX_FRM_SZ(test_req.payload_size), len, i);

		k_sleep(K_SECONDS(1));
	}

	zassert_true(script_runner.result.done, "Script should be done");
	zassert_equal(script_runner.result.ret, -EAGAIN, "Script should time out");
}
...
ZTEST(modem_ubx, test_cmd_rsp_blocks_and_receives_rsp)
{
	...
	modem_backend_mock_put(&mock, (uint8_t *)&test_rsp, UBX_FRM_SZ(test_rsp.payload_size));
	test_thread_yield();
...
ZTEST(modem_ubx, test_script_is_thread_safe)
{
	...
	struct script_runner script_runner_1 = {
		.script = {
			.request = {
				.buf = &test_req,
				.len = UBX_FRM_SZ(test_req.payload_size),
			},
			.match = MODEM_UBX_MATCH_DEFINE(UBX_CLASS_ID_ACK, UBX_MSG_ID_ACK, NULL),
			.response = {
				.buf = cmd_response,
...
ZTEST(modem_ubx, test_rsp_filters_out_bytes_before_payload)
{
	...
	memcpy(buf + buf_len, &test_rsp, UBX_FRM_SZ(test_rsp.payload_size));
	buf_len += UBX_FRM_SZ(test_rsp.payload_size);
	...
	zassert_mem_equal(&test_rsp,
			  script_runner.script.response.buf,
			  UBX_FRM_SZ(test_rsp.payload_size));
}
...
ZTEST(modem_ubx, test_rsp_incomplete_packet_discarded)
{
	...
	memcpy(buf, &test_rsp, UBX_FRM_SZ(test_rsp.payload_size) - 5);
	buf_len += UBX_FRM_SZ(test_rsp.payload_size) - 5;
...
ZTEST(modem_ubx, test_rsp_discards_invalid_len)
{
	...
	/** Invalidate checksum */
	size_t frame_size = UBX_FRM_SZ(test_rsp.payload_size);
...
ZTEST(modem_ubx, test_rsp_filters_out_non_matches)
{
	 ...
	 */
	memcpy(buf, &test_rsp_non_match, UBX_FRM_SZ(test_rsp_non_match.payload_size));
	buf_len += UBX_FRM_SZ(test_rsp_non_match.payload_size);
	 ...
	 */
	memcpy(buf, &test_rsp_match, UBX_FRM_SZ(test_rsp_match.payload_size));
	buf_len += UBX_FRM_SZ(test_rsp_match.payload_size);
	...
	zassert_true(script_runner.result.done, "Script should be done");
	zassert_ok(script_runner.result.ret);
	zassert_equal(UBX_FRM_SZ(test_rsp_match.payload_size),
		      script_runner.script.response.received_len,
		      "expected: %d, got: %d",
		      UBX_FRM_SZ(test_rsp_match.payload_size),
		      script_runner.script.response.received_len);
...
ZTEST(modem_ubx, test_rsp_match_with_payload)
{
	...
	script_runner.script.match.filter.payload.buf = test_rsp_match.payload_and_checksum;
	script_runner.script.match.filter.payload.len = test_rsp_match.payload_size;
...
ZTEST(modem_ubx, test_unsol_matches_trigger_cb)
{
	...
	modem_backend_mock_put(&mock,
			       ...
			       UBX_FRM_SZ(ack_frame.payload_size));
	test_thread_yield();
	...
	modem_backend_mock_put(&mock,
			       ...
			       UBX_FRM_SZ(nak_frame.payload_size));
	test_thread_yield();
