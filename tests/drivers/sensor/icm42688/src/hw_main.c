	...
	for (int i = 0; i < ARRAY_SIZE(channels); i++) {
		...
		while (decoder->decode(buf, ch, &fit, 1, decoded_buffer) > 0) {
			...
			if (ch.chan_type == SENSOR_CHAN_ACCEL_XYZ) {
				...
				zassert(fabs(x) < 1.0, "fail", "accel x out of range: %10.4f, is board level?", x);
				zassert(fabs(y) < 1.0, "fail", "accel y out of range: %10.4f, is board level?", y);
				zassert(fabs(z - 9.8) < 1.0, "fail", "accel z out of range: %10.4f, is board level?", z);
			} else if (ch.chan_type == SENSOR_CHAN_GYRO_XYZ) {
				struct sensor_three_axis_data* data = (struct sensor_three_axis_data*)decoded_buffer;
				double x = q31_to_double(data->readings[0].values[0], data->shift);
				double y = q31_to_double(data->readings[0].values[1], data->shift);
				double z = q31_to_double(data->readings[0].values[2], data->shift);
				zassert(fabs(x) < 0.1, "fail", "gyro x out of range: %10.4f, is board stationary?", x);
				zassert(fabs(y) < 0.1, "fail", "gyro y out of range: %10.4f, is board stationary?", y);
				zassert(fabs(z) < 0.1, "fail", "gyro z out of range: %10.4f, is board stationary?", z);
			}
		}
	}
}
